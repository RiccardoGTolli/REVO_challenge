...
def get_year_quarter_combos(start_year: int, start_quarter: int, end_year: int, end_quarter: int) -> pd.DataFrame:
...
def plot_lin_reg_scatter(df: pd.DataFrame, col: str, coeff: float, pvalue: float, dir: str) -> None:
    ...
    plt.scatter(df["Year"] + (df["Quarter"] - 1) / 4, df[col], color="blue")
