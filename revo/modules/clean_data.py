...
class Dataset:
    ...
    def __init__(
        ...
        data_modified_path: str = "data_modified/",
    ) -> None:
    ...
    def output_task1_and_task2_data(
        self,
        df,
        sector: pd.DataFrame,
        knn_neighbors,
        ...
        df["Quarter"] = df["Quarter"].str.extract(r"(\d+)").astype(float)  # type:ignore
        ...
        df = pd.concat(
            [df[["Country", "description_sector"]], df.drop(["Country", "description_sector"], axis=1).astype(float)],
            axis=1,
        )
