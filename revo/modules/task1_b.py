...
def task1_b(start_year: int, start_quarter: int, end_year: int, end_quarter: int, pvalue: float = 0.05):
    # Import cleaned data
    ...
    for sector in df["description_sector"].unique():
        ...
        sector_df_results = (
            pd.DataFrame(
                {
                    "Statistical Significant Financial Indicator": cols_list,
                    "p-value": pvalues_list,
                    "slope": coefficients_list,
                    "description_sector": sector,
                }
            )
    ...
    task1_b_answer = pd.DataFrame(
        {task1_b_answer.index.name: task1_b_answer.index.values, "Rank": task1_b_answer}
    ).reset_index(drop=True)
    ...
    # Produce plots
    for financial_col, sector in zip(
        all_sectors_df["Statistical Significant Financial Indicator"], all_sectors_df["description_sector"]
    ):
        if financial_col.startswith("MI_"):
            continue  # Skip to the next iteration if the col is a missing indicator

        filtered_df = df[df["description_sector"] == sector].copy()
        filtered_df = filtered_df[[financial_col, f"Predicted {financial_col}", "Year", "Quarter"]]
