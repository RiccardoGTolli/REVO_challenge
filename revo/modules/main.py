...
from task2 import RandomForest_,LightGBM_,XGBoost_
...
def main():
    ...
    # RANDOM FOREST
    rf_=RandomForest_()
    X_train,X_test,y_train,y_test,labels_list=rf_.ml_preprocessing(config['IQR_multiplier'],
                                                                config['knn_neighbors'],
                                                                config['test_size_percentage'],
                                                                df_path='output/0_clean_arff/df_task2.csv')
    rf_accuracy,rf_report=rf_.train_and_return_accuracy(X_train,X_test,y_train,
                                                        y_test,labels_list,config['rf_hyperparameters'])

    # LIGHT GBM
    lgbm_=LightGBM_()
    lgbm_accuracy,lgbm_report=lgbm_.train_and_return_accuracy(X_train,X_test,y_train,
                                                            y_test,labels_list,config['lgbm_hyperparameters'])

    # XGBOOST
    xgb_=XGBoost_()
    xgb_accuracy,xgb_report=xgb_.train_and_return_accuracy(X_train,X_test,y_train,
                                                        y_test,labels_list,config['xgboost_hyperparameters'])

    # Create a dictionary to store variable names, their corresponding accuracies, and reports
    models_results = {'Random Forest': [rf_accuracy, rf_report],
    ...
    print(f"The best model is {best_model} with accuracy {best_accuracy} and report:\n{best_report}")
